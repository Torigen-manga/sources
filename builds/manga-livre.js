...
// node_modules/.pnpm/@torigen+mounter@0.7.2_typescript@5.8.3/node_modules/@torigen/mounter/dist/base/class.js
...
// sources/manga-livre/src/helper.ts
var fromUrl = {
  getMangaIdFromUrl(url) {
...
function statusReturnal(status) {
  switch (status) {
    case "Em Andamento":
      return "Ongoing";
    case "Completo":
      return "Completed";
    case "Em Lan\xE7amento":
      return "Ongoing";
    case "Hiato":
      return "Hiatus";
    default:
...
// sources/manga-livre/src/parser.ts
function parsePopularSection(html3) {
  const $2 = load(html3);
  const entries = [];
  const mangaEntry = $2(".manga-carousel-item");
  ...
  return {
    id: "popular-section",
    title: "Mang\xE1s Populares",
    items: entries,
    type: "SingleRowNormal",
    containsMoreItems: false
  };
}
function parseFeaturedSection(html3) {
  const $2 = load(html3);
  const entries = [];
  const section = $2("section.featured-section");
...
function parseLatestUpdates(html3) {
  const $2 = load(html3);
  const entries = [];
  const section = $2("section.latest-section");
  const entry = section.find("div > article.manga-card");
  entry.each((_, element) => {
    const $el = $2(element);
    const title = $el.find("h3").text().trim();
  ...
  return {
    ...
    title: "\xDAltimas Atualiza\xE7\xF5es",
    items: entries,
    type: "SingleRowNormal",
    containsMoreItems: false
  };
...
function parseMangaDetails(html3) {
  const $2 = load(html3);
  const image = $2("img.manga-cover-image").attr("src") || "";
  const title = $2("h1.manga-title").text().trim();
  const description = $2("div.synopsis-content > p").text().trim();
  const tags = [];
  const authors = [];
  const artists = [];
  let status = "Unknown";
  $2(".manga-tag").each((_, element) => {
    const $el = $2(element);
    const tag = $el.text().trim();
    tags.push({ label: tag, id: tag.toLowerCase().replace(/\s+/g, "-") });
  });
  $2(".manga-meta-item").each((_, element) => {
    const $el = $2(element);
...
function parseChapterList(html3) {
  const $2 = load(html3);
  const chapters = [];
  $2("li.chapter-item").each((_, element) => {
    const $el = $2(element);
...
function parseChapterDetails(html3, chapterId) {
  const $2 = load(html3);
  const pages = [];
  const id = chapterId;
  const raw = $2("h1.manga-title").clone().children("a, div").remove().end().text().trim();
  const title = raw.replace(/^\s*\/\s*/, "").replace(/\s*-\s*/, " - ").replace(/\s+/g, " ");
  $2("img.chapter-image").each((_, element) => {
    const $el = $2(element);
...
// sources/manga-livre/src/index.ts
var capabilities = {
...
var MangaLivreSource = class extends AbstractSource {
  constructor() {
    super(...arguments);
    ...
    __publicField(this, "capabilities", capabilities);
    __publicField(this, "fieldsMetadata", fieldsMetadata);
  }
  async getHomepage() {
    const homeRequest = {
    ...
    const sections = [];
    sections.push(parsePopularSection(html3));
    sections.push(parseFeaturedSection(html3));
    sections.push(parseLatestUpdates(html3));
    return sections;
  }
  ...
  async getChapterDetails(chapterId) {
    ...
    const html3 = await res.text();
    return parseChapterDetails(html3, chapterId);
  }
  getSearchTags() {
    throw new Error("getSearchTags not implemented.");
  }
};
var index_default = MangaLivreSource;
export {
